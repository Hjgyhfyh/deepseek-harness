...
describe('session export compression config', () => {
  it('defaults to level 6 and rejects values outside the integer 0-9 range', () => {
    ...
    expect(ApiProxyService.Config({ sessionExportCompressionLevel: 0 }))
      .toEqual({ sessionExportCompressionLevel: 0, coldBlankProbeMaxBytes: 1024, autoContinueOnMaxTokens: true })
    expect(ApiProxyService.Config({ sessionExportCompressionLevel: 9 }))
      .toEqual({ sessionExportCompressionLevel: 9, coldBlankProbeMaxBytes: 1024, autoContinueOnMaxTokens: true })
...
describe('cold blank probe config', () => {
  it('accepts a per-Session byte bound including zero and rejects invalid bounds', () => {
    expect(ApiProxyService.Config({ coldBlankProbeMaxBytes: 0 }))
      .toEqual({ sessionExportCompressionLevel: 6, coldBlankProbeMaxBytes: 0, autoContinueOnMaxTokens: true })
    expect(ApiProxyService.Config({ coldBlankProbeMaxBytes: 2048 }))
      .toEqual({ sessionExportCompressionLevel: 6, coldBlankProbeMaxBytes: 2048, autoContinueOnMaxTokens: true })
...
describe('autoContinueOnMaxTokens config', () => {
  it('defaults on and accepts an explicit off', () => {
    expect(ApiProxyService.Config({ autoContinueOnMaxTokens: false }))
      .toEqual({ sessionExportCompressionLevel: 6, coldBlankProbeMaxBytes: 1024, autoContinueOnMaxTokens: false })
  })
})
