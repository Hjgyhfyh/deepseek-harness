...
`message queued as the next turn for subagent <subagent_id>` on the default queued path, or `delivered immediately (running turn interrupted) to subagent <subagent_id>` with `deliver: "now"`; the canonical output carries the accepted `messageId` plus the resolved `delivered` mode. `deliver: "now"` interrupts the target's current turn (inbox preserved), waits — bounded by a 10 s cap — for the driver to settle, then admits the message as a waking turn; if the cap expires first, the delivery degrades to the queued path and reports `queued`. A failure — an unauthorized or unknown child, a descriptor-less child that cannot be resumed, or admission rejected — is an errored result whose message states the message was not delivered.
...
- **Default queued delivery does not redirect the current turn** — without `deliver: "now"`, every message opens a later FIFO turn, so a message sent while the child is working runs only after its current turn finishes. `deliver: "now"` redirects immediately by interrupting the current turn first (bounded 10 s settle wait, degrading to the queued path on timeout).
