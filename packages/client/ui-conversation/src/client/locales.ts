...
export const zh = {
  ...
  'input.stop': '停止生成',
  'input.send': '发送消息',
  'input.queue': '排队发送（本轮结束后送达）',
  'input.sendNow': '立即发送（打断当前轮次）',
  'input.continue': '继续',
  'placeholder.steerQueue': 'Cmd/Ctrl+Enter 立即发送全部排队消息',
  ...
  'message.maxTokens.hint': '回答被截断，已有输出保留在对话中。点击「继续」可让模型接着输出。',
...
export const en = {
  ...
  'input.queue': 'Queue message (delivered after the current turn)',
  'input.sendNow': 'Send now (interrupts the running turn)',
  'input.continue': 'Continue',
  'placeholder.steerQueue': 'Cmd/Ctrl+Enter sends all queued messages now',
  ...
  'message.maxTokens.hint': 'The reply was cut off; earlier output is preserved in the conversation. Click Continue to let the model resume.',
  ...
  'queue.steer.unavailable': 'Send now is available only while the agent is running',
  ...
  'queue.steerFailed': 'Send now failed. Try again.',
