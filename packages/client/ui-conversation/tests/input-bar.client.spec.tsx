...
describe('Enter semantics', () => {
  it('advertises the empty-draft whole-queue steering gesture when it is available', () => {
    ...
    expect(textarea.placeholder).toBe('Cmd/Ctrl+Enter 立即发送全部排队消息')
  })
  ...
  it('keeps the owning placeholder or ordinary guidance when whole-queue steering is unavailable', () => {
    ...
    // Subagent sessions now get the same whole-queue gesture while running
    // with an empty draft and queued rows.
    expect(bench({
      running: true,
      queue: [row('q-1')],
      subagent: {
    ...
    }).textarea.placeholder).toBe('Cmd/Ctrl+Enter 立即发送全部排队消息')
    expect(bench({
      running: true,
      queue: [row('q-1')],
      placeholder: '上层指定提示',
  ...
  it('queue steering stays gated: idle, subagent, plain Enter, empty queue, or steering-only rows', () => {
    ...
    // Subagent sessions now flush the queue through the same accelerated chord.
...
describe('running and lock semantics', () => {
  ...
  it('busy non-empty draft renders explicit queue and send-now delivery controls', () => {
    const busy = bench({ running: true, draft: '稍后发' })
    expect(busy.view.getByLabelText('排队发送（本轮结束后送达）')).toBeTruthy()
    expect(busy.view.getByLabelText('立即发送（打断当前轮次）')).toBeTruthy()
  })

  it('queue control keeps the plain-Enter default: delivered after the running turn', () => {
    const { view, sink } = bench({ running: true, draft: '稍后发' })
    fireEvent.click(view.getByLabelText('排队发送（本轮结束后送达）'))
    expect(sink).toHaveBeenCalledWith('稍后发', [], 'queue')
  })

  it('send-now control submits with immediate steer delivery into the running turn', () => {
    const { view, sink } = bench({ running: true, draft: '现在发' })
    fireEvent.click(view.getByLabelText('立即发送（打断当前轮次）'))
    expect(sink).toHaveBeenCalledWith('现在发', [], 'steer')
  })

  it('delivery controls stay gated: idle and empty busy draft (children included)', () => {
    const idle = bench({ draft: '空闲' })
    expect(idle.view.queryByLabelText('排队发送（本轮结束后送达）')).toBeNull()
    expect(idle.view.queryByLabelText('立即发送（打断当前轮次）')).toBeNull()

    const emptyBusy = bench({ running: true })
    expect(emptyBusy.view.queryByLabelText('排队发送（本轮结束后送达）')).toBeNull()
    expect(emptyBusy.view.queryByLabelText('立即发送（打断当前轮次）')).toBeNull()

    // Running continuable children now expose the same delivery pair as
    // ordinary sessions while a draft is present.
    const child = bench({
      running: true,
      draft: '子级',
      subagent: {
    ...
    expect(child.view.getByLabelText('排队发送（本轮结束后送达）')).toBeTruthy()
    expect(child.view.getByLabelText('立即发送（打断当前轮次）')).toBeTruthy()
  })
