...
describe('workspace browser rows', () => {
  ...
  it('shows descendant activity without describing an idle parent as running', () => {
    ...
    try {
      ...
      // The ×N worker badge shows from two subagents up.
  ...
  it('keeps descendant activity secondary while the parent is running', () => {
    ...
    try {
      ...
      render(<SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
        onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} onTogglePin={vi.fn()} t={t} />)
      const row = screen.getByRole('treeitem')
      // One chase dot per live status (own + subagents), the ×N badge on the
      // counted one; both labels stay in the accessibility tree.
      expect(row.querySelectorAll('[data-state="ongoing"]')).toHaveLength(2)
      expect(screen.getByText('进行中')).toBeTruthy()
  ...
  it('keeps child activity as a secondary status while user attention is primary', () => {
    ...
    // The subagent status keeps its own chase dot beside the warning dot.
  ...
  it('session row menu opens without opening the session and dispatches rename, fork, and archive', () => {
    ...
    render(<SessionNodeItem node={node} currentId={undefined} now={0} onOpen={onOpen}
      onRename={onRename} onFork={onFork} onArchive={onArchive} onTogglePin={vi.fn()} t={t} />)
