...
`src/styles/` 下有五张样式表，全部由 web 壳的 `base.css` 导入：`base.css`、`design-platform.css`、`scrollbar.css`、`gradient-shadow-text.css` 与 `shiki.css`。`scrollbar.css` 是 `--dsw-alias-scrollbar-*` token 的唯一消费方，必须排在声明这些 token 的 `design-platform.css` 之后。`base.css` 还声明 `--dsw-motion-*`、`--dsw-radius-*`、浮层 z-index 阶梯 `--dsw-z-*` 以及 `--dsw-shadow-focus-ring`。浮层原语消费该阶梯；文档流内的层叠（时间线、composer 座位）仍用局部整数。
