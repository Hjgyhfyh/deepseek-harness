 ...
 * `window.__MAKO_BOOT__` entry graph (wire single source: {@link WebBootEntry}
...
/**
 * Inject the boot entry graph into index.html: `window.__MAKO_BOOT__` as the
...
export function injectBootManifest(html: string, graph: WebBootGraph): string {
  ...
  // Dual-write across the Mako rename: rebuilt shells read __MAKO_BOOT__, while
  // not-yet-rebuilt hosts and shells still serve/read __DSH_BOOT__. Same graph
  // object under both names keeps either generation bootable.
  const script = `<script>window.__MAKO_BOOT__ = window.__DSH_BOOT__ = ${json}</script>`
