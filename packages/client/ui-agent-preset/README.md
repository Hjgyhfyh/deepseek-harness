...
The browser edits no composition text. Editing YAML in a web textarea was a weak surface (no completion, no highlighting, no diff), so a new preset is a host-side copy of an existing one — the dialog collects an id (it becomes the directory name, which is why it must be named up front and cannot change later) and an optional display name, and `{ from, id, name? }` is all that crosses the wire. Everything else — description, composition, skills — is edited in the preset's own files, and the page's other job is getting the user TO those files: the copy completes by opening the new directory, and every custom row keeps a location action. Where the host has no desktop opener (`hasDocument: false` on the roster; remote and container deployments), the same actions answer the directory as text on the row instead of offering a button that would spawn into nothing. Escape hides that revealed path (`preventDefault`) and restores focus to the location action so Settings can still take the next key; a row that is not showing a path ignores the key. Keyboard focus paints the row actions in the same primary color and fill as hover, and the delete action in the same danger color.
...
Beside copying sits the conversational entry: when the roster carries the self-referential `cordis` preset, a dashed add-card (the Models page's affordance) stages it and starts a new session — the section closes the settings panel through the shell's owner-prop `close` and the new-session chip's own applier composes the blank session the workspace flow produces. Keyboard focus paints the product ring and the same business-blue dash as the no-workspace composer card; hover paints the dash too. A failed roster load offers a retry control with the same ring. The seat keeps a late roster load from regressing the display: staged pick first, then the composition the current session already carries, then the deployment default.
