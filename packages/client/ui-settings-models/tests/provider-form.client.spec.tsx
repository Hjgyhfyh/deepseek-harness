...
describe('model list editing', () => {
  ...
  it('keeps expansion on the row it belongs to after an earlier one is removed', async () => {
    ...
    expect(screen.queryByLabelText(`${en.modelMaxTokens} 1`)).not.toBeNull()
    expect(screen.queryByLabelText(`${en.modelMaxTokens} 2`)).toBeNull()
  })
...
describe('endpoint interrogation', () => {
  ...
  it('folds a row\u2019s capacities away until they are asked for', async () => {
    ...
    // The context window rides the row itself now; only the output cap folds.
    ...
    expandModel(1)
    // The fold closes without hiding the row's own context field.
    expect(screen.getByLabelText(`${en.modelContextWindow} 1`)).toBeTruthy()
    expect(screen.queryByLabelText(`${en.modelMaxTokens} 1`)).toBeNull()
  })
