 ...
 * `window.__MAKO_BOOT__` into the two-view BootManifest (wire boundary)
...
export class AppWebEntry {
  ...
  async run(): Promise<void> {
    // Legacy-name fallback: a host predating the Mako rename still injects
    // only __DSH_BOOT__; prefer __MAKO_BOOT__ when present.
    const win = globalThis as DshWindow
    this.manifest = parseBootManifest(win.__MAKO_BOOT__ ?? win.__DSH_BOOT__)
    ...
    ;(globalThis as DshWindow).__MAKO_MODULES__ = this.modules
