...
This package currently owns the generic fallback and the built-in shell/pwsh, read, write/edit, grep/glob, web, todo, question, and Code Dispatch presentations. `ui-skill` demonstrates a business-owned registration for `skill`. The shell row is a local ToolRow replica: Escape collapses an open card (`preventDefault`) and restores focus to the summary, including from Inspect; a closed row ignores the key. Keyboard focus uses the product ring, shows the same icon→chevron preview as hover, and keeps Inspect visible while the card holds focus.
