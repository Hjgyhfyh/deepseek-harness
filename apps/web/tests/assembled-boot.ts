...
interface FixtureWindow extends Window {
  __MAKO_BOOT__?: { rev: string; entries: WebBootEntry[] }
...
export function mountAssembledApp(): void {
  ...
  win.__MAKO_BOOT__ = { rev: 'fx', entries: PLUGINS.map(({ bundlePath: _bundlePath, ...plugin }) => plugin) }
  act(() => {
