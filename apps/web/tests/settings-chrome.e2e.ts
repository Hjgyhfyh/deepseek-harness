...
describe('web e2e: settings modal and General preferences', () => {
  ...
  it('persists the busy-state Enter behavior across reload and a distinct port', async () => {
    ...
    await page.getByRole('menuitem', { name: '立即发送' }).click()
    await dialog.getByRole('button', { name: '立即发送' }).waitFor({ timeout: 10_000 })
    ...
    await reloaded.getByRole('button', { name: '立即发送' }).waitFor({ timeout: 10_000 })
    ...
    try {
      ...
      await secondPage.getByRole('dialog', { name: '设置' })
        .getByRole('button', { name: '立即发送' }).waitFor({ timeout: 10_000 })
    ...
    await reloaded.getByRole('button', { name: '立即发送' }).click()
