...
- pi-ai credentials, transport knobs, SDK timeouts, and the fifteen-minute-default `streamIdleTimeoutMs` watchdog are scoped per provider profile. Hidden provider retries are disabled; bounded retries belong to the separately composed agent recovery policy.
