...
| `@deepseek-ai/dsh-tool-mail` | `mail_codes`、`mail_list_recent`、`mail_read` | `ctx.tools`、`ctx.mail`、`ctx.systemPrompt` | `tool/call`、`tool/result` | - | mail_codes、mail_list_recent 和 mail_read 将提供方选择置于 ctx.mail 之后，使模型可见 schema 在更换后端时保持稳定。 |
...
<a id="deepseek-aidsh-tool-mail"></a>

## `@deepseek-ai/dsh-tool-mail`

### `mail_codes`

扫描最新邮箱邮件以查找验证或登录码，并返回紧凑的验证码、发件人、主题和 id 行。
...
```json
{
  ...
  "properties": {
    "limit": {
      "type": "integer",
      "description": "How many newest messages to scan (1-10). Omit for the default."
    }
  }
}
```

来源：[`packages/mail/tool-mail/src/index.ts`](../packages/mail/tool-mail/src/index.ts)

### `mail_list_recent`

列出已连接邮箱中的最新邮件，包含发件人、主题、日期以及可供 mail_read 使用的 id。
...
```json
{
  ...
  "properties": {
    "limit": {
      "type": "integer",
      "description": "How many newest messages to return (1-10). Omit for the default."
    }
  }
}
```
...
```json
{
  ...
  "properties": {
    "uid": {
      "type": "string",
      "description": "The message id from mail_list_recent."
    }
  },
  "required": [
    "uid"
...
mail_codes、mail_list_recent 和 mail_read 将提供方选择置于 ctx.mail 之后，使模型可见 schema 在更换后端时保持稳定。
