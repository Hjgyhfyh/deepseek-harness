...
- **Provider stalls are bounded at the transport.** Both shipping remote adapters expose positive finite `streamIdleTimeoutMs` with a fifteen-minute default. The watchdog arms only while iterator `next()` is outstanding, uses one stable signal for the whole request, maps its own expiry to `TIMEOUT`, and keeps an earlier caller abort as `ABORTED`. Adapters pulse the watchdog on out-of-band transport activity (DeepSeek SSE comments; pi-ai `onPayload`, including SSE pings) without yielding those payloads as `StreamChunk` values.
