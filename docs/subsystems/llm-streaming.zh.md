...
- **提供方停顿在传输层受到时限约束。** 两个已交付的远程适配器都暴露正数且有限的 `streamIdleTimeoutMs`，默认十五分钟。watchdog 只在 iterator `next()` 尚未完成时启动，整个请求使用同一个稳定 signal，把自身到期映射为 `TIMEOUT`，并把更早发生的调用方中止保留为 `ABORTED`。适配器会在带外传输活动上脉冲 watchdog（DeepSeek SSE 注释；pi-ai `onPayload`，包括 SSE ping），这些载荷不会成为 `StreamChunk` 值。
